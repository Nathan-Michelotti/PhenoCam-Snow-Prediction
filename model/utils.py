...
def get_site_metadata(site_name):
    """Scrapes metadata from the PhenoCam site info page."""
    metadata_url = f"https://phenocam.nau.edu/webcam/sites/{site_name}/"
    try:
        ...
        for line in lines:
            if "Group:" in line:
                meta["Group"] = line.split("Group:")[1].strip()
            elif "UTC Offset:" in line:
                meta["UTC Offset"] = line.split("UTC Offset:")[1].strip()
            elif "Site Type:" in line:
                meta["Site Type"] = line.split("Site Type:")[1].strip()
            elif "dominant_species" in line:
                meta["dominant_species"] = line.split(":")[-1].strip()
            elif "primary_veg_type" in line:
                meta["primary_veg_type"] = line.split(":")[-1].strip()
            elif "site_notes" in line:
                meta["site_notes"] = line.split(":")[-1].strip()
        return meta
...
def download(site_name, dates, save_to: str | Path, n_photos):
    ...
    log_filename = f'{datetime.now().isoformat().split(".")[0].replace(":", "-")}.log'
    log_filepath = Path(save_dir) / log_filename
    ...
    with open(log_filepath, "a") as log_file:
        ...
        progress = tqdm(date_range, desc=f"{site_name} Downloading", total=n_photos)

        for my_datetime in progress:
            ...
            if resp1.ok:
                ...
                for a in arr:
                    ...
                    output_fpath = Path(save_dir) / img_fname
                    ...
                    if resp2.ok:
                        try:
                            ...
                            img.verify()  # ensure file is a valid image
                            img = Image.open(BytesIO(resp2.content))  # reopen for saving
                            img.save(output_fpath)
                            success = True
                            n_downloaded += 1
                            log_file.write(f"INFO:Retrieved {resp2.url}\n")
                            progress.set_postfix_str(f"{n_downloaded}/{n_photos}")

                            # Save metadata
                            meta = get_site_metadata(site_name)
                            meta['image_filename'] = img_fname
                            meta['timestamp'] = str(pd_timestamp)
                            meta_path = Path(save_dir) / f"{img_fname.replace('.jpg', '')}_meta.txt"
                            with open(meta_path, "w") as mf:
                                for k, v in meta.items():
                                    mf.write(f"{k}: {v}\n")

                        except Exception as e:
                            log_file.write(f"WARN:Could not read or save image from {resp2.url}: {e}\n")
                        break
                    else:
...
def download_direct_image(site_name, date, time, save_to):
    Y, m, D = date.split("-")
    strip_timestamp = time.replace(":", "")
    img_fname = f"{site_name}_{Y}_{m}_{D}_{strip_timestamp}.jpg"
    img_url = f"https://phenocam.nau.edu/data/archive/{site_name}/{Y}/{m}/{img_fname}"
    output_path = Path(save_to) / img_fname
